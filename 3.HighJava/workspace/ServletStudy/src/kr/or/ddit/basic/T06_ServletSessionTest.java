 ...
 *  - 세션을 통해서 사용자(브라우저)별로 구분하여 정보를 관리할 수 있다.(세션ID 사용)
 *  - 쿠키를 사용할 때보다 보안이 향상된다.(정보가 서버에 저장되므로)
 *  - 사용자 당 1개씩 만들어진다.
 * 
 * 2. 세션객체를 가져오는 방법
 *  - HttpSession session = request.getSession(boolean값);
 *  - boolean값 : true - 세션객체가 존재하지 않으면 새로 생성한다.
...
public class T06_ServletSessionTest extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		String userId = "Lee";	//사용자ID
		
		if(session.isNew()) {	//세션이 새로 생성되었는지 확인
			title = "첫 방문을 환영합니다.";
		...
		//세션삭제 : 삭제 시 새로고침을 해도 방문횟수가 누적되지 않고, 세션ID는 계속 변경된다.
		//방법1 : 세션을 삭제한다.
		//session.invalidate();
		//방법2 : 설정한 시간(5초) 동안 세션이 유지된다.
