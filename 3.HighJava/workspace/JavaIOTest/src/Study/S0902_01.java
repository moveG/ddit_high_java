...
public class S0902_01 {
	...
	public static void main(String[] args) throws IOException {
		//1. new File(String 파일또는경로명)
		File f1 = new File("d:/EE/text.txt");
		System.out.println("파일명 : " + f1.getName());
		//getName() : 파일이름 추출
		System.out.println("파일 여부 : " + f1.isFile());
		//isFile() : 파일 존재 여부를 판단해 true 또는 false를 반환
		System.out.println("파일 여부 : " + f1.isDirectory());
		//isDirectory() : 디렉토리 존재 여부를 판단해 true 또는 false를 반환
		System.out.println("===============================");
		
		File f2 = new File("d://EE");
		System.out.print(f2.getName() + "은 ");
		if(f2.isFile()) {
		...
		//2. new File(File parent, String child)
		File f3 = new File(f2, "text.txt");
		//f2는 디렉토리("d://EE")를 가지고 있다.
		//EE폴더 아래의 test.txt파일을 file3에 넣음
		System.out.println(f3.getName() + "의 용량 크기 : " + f3.length() + "bytes");
		//파일.length() : 파일의 크기를 bytes타입으로 반환
		
		
		//3. new File(String parent, String child)
		File f4 = new File("d:/EE/test/..", "test.txt");
		System.out.println("절대 경로 : " + f4.getAbsolutePath());
		System.out.println("경로        : " + f4.getPath());
		System.out.println("표준 경로 : " + f4.getCanonicalPath());
		...
		//2. mkdirs()
		File f6 = new File("d:/EE/test/java/src");
		if(f6.mkdir()) {
