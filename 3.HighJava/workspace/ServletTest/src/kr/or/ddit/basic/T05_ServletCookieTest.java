 ...
 * 
 * 3. 동작방식
 *  - 쿠키생성단계 : 생성한 쿠키를 응답 데이터의 헤더에 저장하여 웹브라우저에 저장한다.
 *  - 쿠키저장단계 : 웹브라우저는 응답 데이터에 포함된 쿠키를 쿠키 저장소에 보관한다.
...
public class T05_ServletCookieTest extends HttpServlet{
	 ...
	 * 
	 * 1. 쿠키 객체를 생성한다.
	 *  - 쿠키값은 사용불가 문자(공백, []()=,"/?@:;)를 제외한 나머지 출력 가능한 아스키 문자 사용할 수 있다.
	 *  - 영어 이외의 문자(ex)한글) 사용시에는 URLEncoder.encode()를 사용하여 인코딩 처리를 해주면 된다.
	 *  - Cookie cookie = new Cookie("key값", "value값");
	 * 
	 * 2. 쿠키 최대 지속시간(초단위)을 설정한다.
	...
	private void setCookieExam(HttpServletRequest req, HttpServletResponse resp) 
		...
		//쿠키 소멸시간(초단위) 설정 : 지정하지 않으면 웹브라우저 종료와 함께 쿠키도 삭제된다.
