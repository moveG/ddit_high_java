...
public class MultiChatServer {
	...
	class ServerReceiver extends Thread {
		...
		public void run() {
			try {
				//서버에서는 클라이언트가 보내는 최초의 메시지인 대화명을 수신해야 한다.
				name = dis.readUTF();
...
//				e.printStackTrace();
	...
	private void sendWhisper(String whisper, String toName) {
		try {
			DataOutputStream dos = new DataOutputStream(
					clients.get(toName).getOutputStream());
			
			dos.writeUTF(whisper);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	private void sendMessage(String msg, String from) {
		...
		if(message.length() > 1 && message.substring(0, 2).equals("/w")) {
			int sidx = message.indexOf(" ") + 1;
			int eidx = message.indexOf(" ", sidx);
			
			String toName = message.substring(sidx, eidx);
			String wMessage = message.substring(eidx + 1);
			String whisper = "[" + from + "]" + wMessage;
			sendWhisper(whisper, toName);
		} else {
			String allMessage = "[" + from + "]" + message;
