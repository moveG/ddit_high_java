package Study;

public class S0827_01 {
	/*
	 * enum(열거형) : 상수값을 선언하는 방법
	 * -static final int A = 0;
	 * -static final int B = 1;
	 * -static final int C = 2;
	 * -static final int D = 3;
	 * 
	 * 위의 것들을 이렇게 바꿔줄 수 있다.
	 * - enum Data (A, B, C, D);
	 * 
	 * 열거형을 선언하는 방법
	 * - enum 열거형 이름 {상수값1, 상수값2, 상수값3, ..., 상수값n};
	 */
	
	//City 열거형 객체 선언(기본값을 이용하는 열거형)
	//생성자를 쓰지 않았지만 컴파일러가 자동으로 생성자를 선언해줌
	public enum City {서울, 부산, 대구, 광주, 대전};
	public enum MyCity {서울, 부산, 대구, 광주, 대전};
	
	//데이터 값을 임의로 지정한 열거형 객체 선언
	//데이터 값을 정해줄 경우에는 생성자를 생성자를 만들어서 괄호 속의 값이 변수에 저장되도록 해야한다.
	public enum Season {
		봄("3월부터 5월까지"),
		여름("6월부터 8월까지"),
		가을("9월부터 11월까지"),
		겨울("12월부터 2월까지");
		
		//괄호 속의 값이 저장될 변수 선언
		private String str;
		
		//생성자 만들기(열거형 생성자는 제어자가 묵시적으로 'private'이다.
		Season(String data){
			this.str = data;
		}
		
		//값을 반환하는 메서드 작성
		public String getStr() {
			return str;
		}
	}
	
	public static void main(String[] args) {
		/*
		 * 열거형에서 사용되는 메서드
		 * - name() : 열거형 상수의 이름을 문자열로 반환한다.
		 *   ex) 서울, 부산, 대구 등의 상수도 객체화하여 사용함
		 * - ordinal() : 열거형 상수가 정의된 순서값을 반환한다.(기본적으로 0부터 시작)
		 * - valueOf("열거형상수이름") : 지정된 열거형에서 "열거형상수이름"과 일치하는 열거형 상수객체를 반환한다.
		 */
		City myCity1;	//열거형 객체변수 선언
		City myCity2;	//열거형 객체변수 선언
		
		//열거형 객체변수에 값 저장하기
		myCity1 = City.서울;
		myCity2 = City.valueOf("서울");
		
		System.out.println("myCity1 : " + myCity1.name());
		//toString()이 자동으로 호출되기 때문에 myCity1.name()대신 myCity1만 넣어도 됨
		System.out.println("myCity1의 ordinal : " + myCity1.ordinal());
		System.out.println();
		
		System.out.println("myCity2 : " + myCity2.name());
		System.out.println("myCity2의 ordinal : " + myCity2.ordinal());
		System.out.println("=============================");
		
		Season ss = Season.valueOf("여름");
		System.out.println("name : " + ss.name());
		System.out.println("ordinal(순서) : " + ss.ordinal());
		System.out.println("get메서드 호출 : " + ss.getStr());
		System.out.println("=============================");
		
		//열거형이름.values() : 데이터를 배열로 가져온다.
		Season[] enumArr = Season.values();
		for(int i = 0; i < enumArr.length; i++) {
			System.out.println(enumArr[i].name() + " : " + enumArr[i].getStr());
		}
		System.out.println("=============================");
		
		City city = City.대구;
		System.out.println(city == City.대전);
		//City타입의 대구와 City타입의 대전과 비교함
		//서로 다른 값이므로 false를 반환함
		
//		System.out.println(city == MyCity.대구);
		//City.대구 대신 MyCity.대구를 넣으면 컴파일 오류가 발생함
		//대구라는 이름은 동일하지만 타입이 City와 MyCity로 서로 다르므로 비교가 불가능함
		//enum을 사용하면 타입안전한 코드를 작성할 수 있음
		
		System.out.println("대구 => " + city.compareTo(City.대구));
		System.out.println("서울 => " + city.compareTo(City.서울));
		System.out.println("대전 => " + city.compareTo(City.대전));
		//compareTo : 비교
		//여기서는 ordinal을 비교함
	}
}
